import os
import json
from datetime import datetime, timedelta
import pytz
from google.oauth2 import service_account
from googleapiclient.discovery import build

SPREADSHEET_ID = "1jHV2OZfsclDfC4Irl5S0FNwxZELzsuiz3edKp2SIb94"
SHEET_TAB_NAME = "Order数と配送計画_v2"
DATE_COLUMN = "D"
GOOGLE_SHEETS_CREDENTIALS = os.environ["GOOGLE_SHEETS_CREDENTIALS"]
SLACK_WEBHOOK_URL = os.environ["SLACK_WEBHOOK_URL"]

import requests

def get_sheets_service():
    credentials_info = json.loads(GOOGLE_SHEETS_CREDENTIALS)
    credentials = service_account.Credentials.from_service_account_info(
        credentials_info,
        scopes=["https://www.googleapis.com/auth/spreadsheets"],
    )
    return build("sheets", "v4", credentials=credentials)

def send_slack(message):
    requests.post(SLACK_WEBHOOK_URL, json={"text": message})

def main():
    pt = pytz.timezone("America/Los_Angeles")
    now_pt = datetime.now(pt)
    yesterday_pt = (now_pt - timedelta(days=1)).date()

    print(f"検索対象日: {yesterday_pt}")

    service = get_sheets_service()

    # D列の値を全て取得
    range_name = f"{SHEET_TAB_NAME}!{DATE_COLUMN}:{DATE_COLUMN}"
    result = service.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID,
        range=range_name,
    ).execute()

    values = result.get("values", [])
    print(f"D列の取得件数: {len(values)}")

    # 最初の10行と最後の10行を表示
    msg = f"🔍 Sheetsデバッグ結果\n"
    msg += f"検索対象日: {yesterday_pt}\n"
    msg += f"D列取得件数: {len(values)}\n\n"
    msg += "【D列の値サンプル（最初の10行）】\n"
    for i, row in enumerate(values[:10]):
        msg += f"行{i+1}: '{row[0] if row else '(空)'}'\n"

    msg += "\n【D列の値サンプル（行400〜410付近）】\n"
    for i, row in enumerate(values[398:410]):
        actual_row = 399 + i
        msg += f"行{actual_row}: '{row[0] if row else '(空)'}'\n"

    # 検索フォーマットの確認
    msg += f"\n【検索フォーマット一覧】\n"
    formats = [
        yesterday_pt.strftime("%m/%d"),
        yesterday_pt.strftime("%-m/%-d"),
        yesterday_pt.strftime("%Y/%m/%d"),
        yesterday_pt.strftime("%-m/%-d/%Y"),
        yesterday_pt.strftime("%m/%d/%Y"),
    ]
    for f in formats:
        msg += f"'{f}'\n"

    print(msg)
    send_slack(msg)

if __name__ == "__main__":
    main()
